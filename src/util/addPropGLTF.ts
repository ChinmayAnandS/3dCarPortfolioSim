...
export default function addPropGLTF(scene: THREE.Scene, propPosition: Position, propScale: Scale, propRotation: Rotation, propPath: string) {
    const loader = new GLTFLoader();
    loader.load(propPath, (gltf) => {
        const propMesh = gltf.scene;
        propMesh.scale.set(propScale.x, propScale.y, propScale.z);
        propMesh.quaternion.setFromAxisAngle(new THREE.Vector3(propRotation.x, propRotation.y, propRotation.z), propRotation.angle);
        propMesh.position.set(propPosition.x, propPosition.y, propPosition.z);
        scene.add(propMesh);
    });
}