...
export default class planeSetup {
  ...
  constructor(
    ...
    this.addBorder();
    this.addPropsGLTF(
      "./assets/playground/trees/twinTrees.glb",
      { x: 17.5, y: 0, z: 15 },
      { x: 1, y: 1, z: 1 },
      { x: 0, y: Math.PI / 2, z: 0 },
    );
    ...
    this.addPropsGLTF(
      "./assets/playground/plane/grndFin3.glb",
      { x: 0, y: -0.5, z: 0 },
      { x: 1, y: 1, z: 1 },
      { x: 0, y: Math.PI, z: 0 },
    );
    ...
    this.addPropsGLTF(
      "./assets/playground/miscelleneous/controlInstructions.glb",
      { x: -55, y: 2, z: -22 },
      { x: 1, y: 1, z: 1 },
      { x: 0, y: -Math.PI / 4, z: 0 },
    );
  ...
  private socialsSetup(position: Position, scale: Scale) {
    this.addPropsGLTF(
      "./assets/socials/github.glb",
      { x: position.x, y: position.y, z: position.z },
      scale,
      { x: Math.PI / 2, y: 0, z: Math.PI / 2 },
    );
    this.createCannonBody(
      new CANNON.Cylinder(1.8, 1.8, 1, 8),
      { x: position.x, y: position.y, z: position.z },
      { x: 0, y: 0, z: 1, angle: Math.PI / 2 },
      0,
      "github",
    );
    this.addPropsGLTF(
      "./assets/socials/linkedIn.glb",
      { x: position.x, y: position.y, z: position.z - 10 },
      scale,
      { x: Math.PI / 2, y: 0, z: Math.PI / 2 },
    );
    this.createCannonBody(
      new CANNON.Box(new CANNON.Vec3(0.3, 1.5, 1.5)),
      { x: position.x, y: position.y, z: position.z - 10 },
      { x: 0, y: 0, z: 0, angle: 0 },
      0,
      "linkedIn",
    ...
    this.addPropsGLTF(
      "./assets/socials/X.glb",
      { x: position.x, y: position.y, z: position.z - 30 },
      scale,
      { x: Math.PI / 2, y: 0, z: Math.PI / 2 },
    );
    this.createCannonBody(
  ...
  private addBorder(): void {
    ...
    const borders = [
      {
        ...
        z: -borderLength / 2,
        sizeX: borderLength,
        sizeY: borderThickness,
        sizeZ: borderThickness,
      }, // Top border
      {
        x: 0,
        y: -borderThickness / 2,
    ...
    borders.forEach((border) => {
      const borderShape = new CANNON.Box(
        new CANNON.Vec3(border.sizeX / 2, border.sizeY / 2, border.sizeZ / 2),
      );
      const borderBody = new CANNON.Body({
        mass: 0,
        material: this.groundBodyMaterial,
  ...
  private onDocumentMouseClick(event: MouseEvent): void {
    ...
    if (intersects.length > 0) {
      ...
      if (clickedObject.name.includes("email")) {
        ...
        window.open(
          "https://www.linkedin.com/in/chinmay-anand-s-a61162202",
          "_blank",
        );
