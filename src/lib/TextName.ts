...
export default class TextName {
  ...
  constructor(
    ...
    const textboxGroundContactMaterial = new CANNON.ContactMaterial(
      this.groundBodyMaterial,
  ...
  private initFirstNameText(): void {
    ...
    loader.load(
      "./assets/playground/miscelleneous/firstName1.glb",
      (gltf) => {
  ...
  private initLastNameText(): void {
    const loader = new GLTFLoader();
    loader.load(
      "./assets/playground/miscelleneous/lastName1.glb",
      (gltf) => {
        this.model1 = gltf.scene.children[0] as THREE.Mesh;
        this.model1.scale.set(1.5, 1.5, 1.5);
        this.model1.position.set(
          this.textPosition.x,
          this.textPosition.y,
          this.textPosition.z + 5.5,
        );
  ...
  private createCannonBody1(): void {
    ...
    this.body1 = new CANNON.Body({
      mass: 10,
      position: new CANNON.Vec3(
        this.model1.position.x,
        this.model1.position.y + 2,
        this.model1.position.z,
      ),
      material: this.textboxMaterial,
      shape: shape,
    });
    this.body1.quaternion.setFromAxisAngle(
      new CANNON.Vec3(0, 1, 0),
      -Math.PI / 2,
    );
  ...
  private createCannonBody2(): void {
    ...
    this.body2 = new CANNON.Body({
      ...
      position: new CANNON.Vec3(
        this.model1.position.x,
        this.model1.position.y + 2,
        this.model1.position.z + 3,
      ),
      material: this.textboxMaterial,
      shape: shape,
    });
    this.body2.quaternion.setFromAxisAngle(
      new CANNON.Vec3(0, 1, 0),
      -Math.PI / 2,
    );
  ...
  public updateTextPhysics(): void {
    ...
    this.model1.quaternion.copy(
      this.body1.quaternion as unknown as THREE.Quaternion,
    );
    ...
    this.model2.quaternion.copy(
      this.body2.quaternion as unknown as THREE.Quaternion,
    );
  }
}
