...
export default class Bricks {
  ...
  constructor(
    ...
    const brickContactMaterial = new CANNON.ContactMaterial(
      this.brickPhysicsMaterial,
      this.brickPhysicsMaterial,
      {
    ...
    const groundBrickContactMaterial = new CANNON.ContactMaterial(
      groundBodyMaterial,
      this.brickPhysicsMaterial,
      {
        friction: 0.9, // High friction to prevent sliding
  ...
  private createStackedBricks(
    brickGeometry: THREE.BoxGeometry,
    brickMaterial: THREE.MeshBasicMaterial,
    bx: number,
    by: number,
  ): void {
    for (let i = 0; i < this.rows; i++) {
      for (let j = 0; j < this.columns; j++) {
        const x =
          (j - this.columns / 2) * (bx + this.brickOffset) +
          this.wallPosition.x;
  ...
  private createStaggeredBricks(
    ...
    for (let i = 0; i < this.rows; i++) {
      ...
      for (let j = 0; j < rowColumns; j++) {
        const x =
          (j - rowColumns / 2) * (bx + this.brickOffset) + this.wallPosition.x;
        const y = (i + 0.5) * (by + this.brickOffset) + this.wallPosition.y;
        const z = this.wallPosition.z;
  ...
  private createTriangularBricks(
    brickGeometry: THREE.BoxGeometry,
    brickMaterial: THREE.MeshBasicMaterial,
    bx: number,
    by: number,
