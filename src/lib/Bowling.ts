...
export default class Bowling {
  ...
  constructor(
    scene: THREE.Scene,
    world: CANNON.World,
    groundBodyMaterial: CANNON.Material,
    ballPosition: Position,
    pinsPosition: Position,
    ballRadius: BallDimensions,
    pinDim: PinDimensions,
    rows: number,
  ) {
    this.scene = scene;
    this.world = world;
    ...
    const pinPinContactMaterial = new CANNON.ContactMaterial(
      this.pinPhysicsMaterial,
      this.pinPhysicsMaterial,
      {
        friction: 0.9,
  ...
  private initBowling(): void {
    ...
    AddPropGLTF(
      this.scene,
      { x: this.ballPosition.x, y: 0, z: this.ballPosition.z + 20 },
      { x: 1, y: 1, z: 1 },
      { x: 0, y: 1, z: 0, angle: Math.PI / 2 },
      "./assets/playground/bowling/fenceStructure.glb",
    );
    //add cannon box for sequence of fence
    AddCannonBox(
      this.world,
      { x: this.ballPosition.x - offset, y: 0, z: this.ballPosition.z + 18.5 },
      { x: 35, y: 2.5, z: 0.3 },
      { x: 0, y: 1, z: 0, angle: Math.PI / 2 },
    );
    AddCannonBox(
    ...
    AddCannonBox(
      this.world,
      {
        ...
        y: 0,
        z: this.pinsPosition.z + 2 * offset + 2.5,
      },
      { x: 0.3, y: 2.5, z: 8 },
      { x: 0, y: 1, z: 0, angle: Math.PI / 2 },
    );
  ...
  private loadModels(): void {
    ...
    this.GLTFloader.load(
      "./assets/playground/bowling/bowlingBall.glb",
      (gltf) => {
    ...
    this.GLTFloader.load(
      "./assets/playground/bowling/bowlingPin.glb",
      (gltf) => {
        const pinMesh = gltf.scene;
        pinMesh.position.y = 0;
        pinMesh.scale.set(0.7, 0.7, 0.7);
  ...
  private createPin(x: number, y: number, z: number): void {
    ...
    const pinShape = new CANNON.Cylinder(
      this.pinDim.topRadius,
      this.pinDim.bottomRadius,
      this.pinDim.height,
      this.pinDim.pinSegments,
    );
    const pinBody = new CANNON.Body({
      mass: 1,
      material: this.pinPhysicsMaterial,
    });
  ...
  public update(): void {
    ...
    for (let i = 0; i < this.pins.length; i++) {
      this.pins[i].position.copy(
        this.pinBodies[i].position as unknown as THREE.Vector3,
      );
      this.pins[i].quaternion.copy(
        this.pinBodies[i].quaternion as unknown as THREE.Quaternion,
      );
    }
  }
}
